...
static int32_t android_create_node(int32_t nodeType)
{
    ...
    case UI_NODE_IMAGE:
        ...
        break;
    case UI_NODE_CAMERA: {
        /* Camera preview: use a TextureView as a Surface target.
         * The actual camera session is managed by the camera bridge,
         * which attaches its preview to the first TextureView it finds. */
        jclass texViewClass = (*env)->FindClass(env, "android/view/TextureView");
        if (texViewClass) {
            jmethodID texViewCtor = (*env)->GetMethodID(env, texViewClass,
                "<init>", "(Landroid/content/Context;)V");
            view = (*env)->NewObject(env, texViewClass, texViewCtor, g_activity);
            (*env)->DeleteLocalRef(env, texViewClass);
        } else {
            LOGE("TextureView class not found — falling back to plain View");
            (*env)->ExceptionClear(env);
            jclass viewClass = (*env)->FindClass(env, "android/view/View");
            jmethodID viewCtor = (*env)->GetMethodID(env, viewClass,
                "<init>", "(Landroid/content/Context;)V");
            view = (*env)->NewObject(env, viewClass, viewCtor, g_activity);
